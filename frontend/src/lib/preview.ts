...
export function previewDocument(html: string, answer: string, mode: Problem['mode']) {
  ...
  const bodyPadding = mode === 'stylesheet' ? '0' : '24px'
  return `<!doctype html><html><head>
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; img-src data:; font-src data:">
  <style>
    :root{color-scheme:dark}
    *:not([data-preview]){box-sizing:border-box}
    body{margin:0;padding:${bodyPadding};font:15px/1.5 system-ui;color:#e8e3f0;background:#121017}
